...
summary.tsco <- function(object, ...) {
  ...
  ll1 <- object$logLik_stage1
  ll2 <- object$logLik_stage2
  ...
  bic1 <- if (is.finite(ll1) && is.finite(df1)) -2 * ll1 + log(object$n_stage1_obs) * df1 else NA_real_
  bic2 <- if (is.finite(ll2) && is.finite(df2)) -2 * ll2 + log(object$n_obs) * df2 else NA_real_
...
print.summary.tsco <- function(
  ...
  cat("\nSample size:\n")
  cat("  N: ", x$n, "\n", sep = "")
  cat("  N contributing to stage 1: ", x$n_stage1, "\n", sep = "")

  cat("\nComponent fit statistics:\n")
  fit_print <- x$component_fit
  numeric_cols <- c("df", "logLik", "AIC", "BIC")
